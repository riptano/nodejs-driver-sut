...
BluebirdPromise.promisifyAll(cassandra.Client.prototype);
...
async.series([
  ...
  function warmup(seriesNext) {
    async.times(100, function (n, next) {
      ...
      async.series([
        function (warmupNext) {
          client.executeAsync(insertQuery, params, { prepare: 1, consistency: types.consistencies.all})
            .then(result => warmupNext(null, result))
            .catch(err => warmupNext(err));
        },
        function (warmupNext) {
          client.executeAsync(selectQuery, selectParams, { prepare: 1, consistency: types.consistencies.all})
            .then(result => warmupNext(null, result))
            .catch(err => warmupNext(err));
        }
      ], next);
  ...
  function insert(seriesNext) {
    ...
    utils.timesSeries(options.series, function (n, nextIteration) {
      ...
      utils.timesLimit(options.ops, limit, function (i, next) {
        ...
        client.executeAsync(insertQuery, params, { prepare: true, consistency: types.consistencies.any})
          .then(() => {
  ...
  function select(seriesNext) {
    ...
    async.timesSeries(options.series, function (n, nextIteration) {
      ...
      async.timesLimit(options.ops, limit, function (n, next) {
        ...
        var queryStart = currentMicros();
        client.executeAsync(selectQuery, params, { prepare: true, consistency: types.consistencies.any})
          .then(() => {
            var duration = currentMicros() - queryStart;
            seriesTimer.update(duration);
            totalTimer.update(duration);
